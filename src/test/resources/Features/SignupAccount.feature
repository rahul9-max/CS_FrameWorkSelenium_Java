...
Scenario: Verify navigation and product display for brand pages
 ...
 @SearchAndCartAfterLogin
   ...
   @AddProductReview
	  Scenario: User adds a review for a product
    Given User launches the browser
    And User navigates to "http://automationexercise.com"
    Then Home page should be visible successfully
    When User clicks on 'Products' button for review
    Then User should be navigated to 'ALL PRODUCTS' page successfully on review page
    When User clicks on 'View Product' button for seeing review
    Then 'Write Your Review' section should be visible for review
    When User enters name, email and review
    And User clicks on 'Submit' button for posting review
    Then Success message 'Thank you for your review.' should be visible on review page
    
  @RecommendedItemsCart
   Scenario: Verify that recommended items can be added to cart
    Given User launches the browser
    And User navigates to "http://automationexercise.com"
    Then Home page should be visible successfully
